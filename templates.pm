...
# This function returns a list [ text, template invocation, text, ... ]

#sub splitTemplateInclude(\$) {
#  my ($refToText) = @_;
#  
#  return split(/$templateRegex/s, $$refToText);
#}
...
sub splitTemplateInclude(\$) {
  ...
  for my $token ( split(/([\{\}])/, $$refToText ) ) {
    #push( @outtokens, $token );
    #next; 
    if( $token eq '{' ) {
      $level ++;
    } elsif( $token eq '}' ) {
    ...
    my $tail = substr( $accumulator, -2 );

    # print("LVL: $level TOK: $token ACC: $accumulator TAIL: $tail\n");

    if( $invocation == 0 and $tail eq '{{' ) {
      # print( "TEX: $accumulator\n");
      push( @outtokens, substr( $accumulator, 0, -2 ) );
      $invocation = 1;
      $accumulator = "{{";
      $level = 2;
    } elsif( $invocation == 1 and $level == 0 and $tail eq '}}' ) {
      # print( "INV: $accumulator\n");
      push( @outtokens, $accumulator );
      $invocation = 0;
      $accumulator = "";
    }
  }
